...
namespace SiteCoreTrainings.Infrastructure
{
    public class GoogleMapField : Control, IContentField
    {
        ...
        private string GetGeoData(BaseItem item, ref string centre)
        {
            ...
            location = item[AddressFieldName];
    ...
    string.Format(
          !string.IsNullOrEmpty(location)
               ...
               : "http://maps.googleapis.com/maps/api/staticmap?center={4}&zoom={1}&size={2}x{3}&sensor=false&maptype=roadmap&markers=color:blue%7Clabel:1%7C{0}&key=AIzaSyA9Tonr3a48iyB-V0WVRlHQe8dbBKjYnNk",
               location, _mapZoomFactor, MAP_WIDTH, MAP_HEIGHT, centre);
        }
        //http://maps.googleapis.com/maps/api/staticmap?&q=1836+S+Broad+StPhiladelphia,+PA+19145-2303&zoom=10&size=300x600&sensor=false&maptype=roadmap&markers=color:blue%7Clabel:1%7C1836+S+Broad+StPhiladelphia,+PA+19145-2303
